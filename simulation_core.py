...
class SimulationCore:
    ...
    def _load_sensors(self, filename):
        ...
        try:
            ...
            print(tr(f"⚠ Error loading sensors {filename}: {e}. Using an empty list.",
                       f"⚠ Ошибка загрузки сенсоров {filename}: {e}. Используем пустой список."))
            return []
...
if __name__ == "__main__":
    print(tr("🚀 Starting simulation core performance test...", "🚀 Запуск теста производительности ядра симуляции..."))
    ...
    print(tr(f"⏳ Simulating {days_to_simulate} days ({total_steps} steps)...", f"⏳ Симуляция {days_to_simulate} суток ({total_steps} шагов)..."))
    ...
    print(tr(f"⏱ Execution time: {duration:.4f} sec", f"⏱ Время выполнения: {duration:.4f} сек"))
    print(tr(f"⚡ Speed: {fps:.1f} steps/sec (Steps Per Second)", f"⚡ Скорость: {fps:.1f} шагов/сек (Steps Per Second)"))
    print(tr(f"ℹ️ This means that 1 year of training (35k steps) will take ~{35040/fps/60:.1f} minutes.", f"ℹ️ Это значит, что 1 год обучения (35k шагов) займет ~{35040/fps/60:.1f} минут."))